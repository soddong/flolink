...
public interface PlantHistoryRepository extends JpaRepository<PlantExpHistory, Integer> {
	...
	@Query("SELECT h FROM PlantExpHistory h WHERE h.plant.plantId = :plantId AND FUNCTION('YEAR', h.dateMonth) = :year")
	List<PlantExpHistory> findByPlantIdAndYear(Integer plantId, Integer year);
